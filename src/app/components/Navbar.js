    ...
    <header className='bg-white border-b-2 border-black'>
      <nav className='mx-auto flex items-center justify-end p-6 lg:p-8 ' aria-label='Global'>
        ...
        <div className='hidden lg:flex lg:gap-x-8 xl:gap-x-12'>
          {navigation.map((item) => (
            <Link key={item.name} href={item.href} className='text-xl xl:text-2xl font-medium uppercase leading-6 tracking-wide text-gray-900'>
              {item.name}
            </Link>
          ))}
        </div>
        <div className='hidden lg:flex ml-20'>
          <Link href='#contact' className='bg-stone-900 uppercase px-3.5 py-2.5 text-md font-medium text-gray-50 shadow-sm hover:bg-neutral-800'>
